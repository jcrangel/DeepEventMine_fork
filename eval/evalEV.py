...
# generate event output and evaluation
def get_events(fids, all_ent_preds, all_words, all_offsets, all_span_terms, all_span_indices, all_sub_to_words,
                 all_ev_preds, g_entity_ids_, params, result_dir):
    # generate predicted entities
    pred_ents = generate_entities(fids=fids,
                                  all_e_preds=all_ent_preds,
    ...
    # write output to file
    ids2ent_tri_eve = events2list(preds_output, pred_ents, result_dir, g_entity_ids_, params)

    return ids2ent_tri_eve

# write events to file
def events2list(pred_output, pred_ents, result_dir, g_entity_ids_, params):
    rev_type_map = params['mappings']['rev_type_map']
    ...
    for fid, preds in pred_output.items():
        ...
        enid_mapping, en_preds_out_, a2_ents_ = mapping_entity_id(
            pred_ents[fid], g_entity_ids_[fid], rev_type_map, params)
        ...
        for event_ in events:
            ...
            if len(args_output) > 0:
                output = ''.join(
                    ['E', str(evid_out), '\t', typeEV, ':', trid, ' ', args_output, '\n'])
                ev_lines.append(output)

            # no argument
            else:
                output = ''.join(
                    ['E', str(evid_out), '\t', typeEV, ':', trid, '\n'])
                ev_lines.append(output)
        ...
        if len(mod_list) > 0:
            for mod_id, mod_data in enumerate(mod_list):
                mod_type = mod_data[0]
                evid_out = mod_data[1]
                output = ''.join(['M', str(mod_id + 1), '\t',
                                 mod_type, ' ', 'E', str(evid_out), '\n'])
                ev_lines.append(output)

        #Dont insert empty data
        if a2_en_lines_ or a2_tr_lines_ or ev_lines:
            ids2ent_tri_eve[fid] = [a2_en_lines_,a2_tr_lines_,ev_lines]
